...
outputDir <- "water/split"
inputDir <- "water"
splitRasters(inputDir, outputDir, 4)

# 2nd: reproject the data to the same projection used by sits
...
classificationFile <- getTifFiles("cerrado-2019-08")[1]
...
outputname <- baseDir("masks/cerrado/cerradoWaterMask.tif")
raster::writeRaster(result2, outputname)
